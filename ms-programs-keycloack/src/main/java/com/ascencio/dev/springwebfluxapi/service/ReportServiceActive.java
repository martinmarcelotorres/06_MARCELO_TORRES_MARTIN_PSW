...
@Service
public class ReportServiceActive {
    ...
    private Mono<byte[]> generateAndSaveReport(List<Programs> programs, String reportFormat) {
        return Mono.fromCallable(() -> {
            // Filtrar solo los programas con condition 'A' (activo)
            List<Programs> filteredPrograms = programs.stream()
                    .filter(program -> "A".equals(program.getCondition()))
                    .collect(Collectors.toList());

            // load file and compile it
            File file = ResourceUtils.getFile("classpath:programs_active.jrxml");
